...
- **Business anomaly 3D tracker** — unclassified fixture tracker for U.S. entity types with P1 queues, taxonomy-mapped improvements (65 annex seeds → 15,080 generated), §8 research agenda, and a rotating **3D anomaly chamber**. Default verify mode filters to **Black-owned** fixture-attested entities only (not live SBA/CERT). Hardened **Black-owned scan bot** discovers candidates from staged channels and **auto-queues them on discover** (24 seed + 36 discovery-pool fixtures, 50+ hardening gates), then searches every queued company against a **business-crime DB** (52 violation categories + 60 case typologies). Every company has a **May forensic popup menu**. A 24/7 **error scout bot** inspects payload integrity and auto-heals without removing features. Closest public installs only. Open `/tracker`.
