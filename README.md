...
- **Business anomaly 3D tracker** — unclassified fixture tracker for U.S. entity types with P1 queues, taxonomy-mapped improvements (65 annex seeds → 10,080 generated), §8 research agenda, and a rotating **3D anomaly chamber**. Default verify mode filters to **Black-owned** fixture-attested entities only (not live SBA/CERT). Hard-coded **Black-owned scan bot** runs 24/7 and searches every scanned company against a **business-crime DB** (52 violation categories + 60 case typologies from SEC/DOJ/FBI narrative fixtures), logging and documenting hits — not live enforcement feeds. Every company has a **May forensic popup menu**. A 24/7 **error scout bot** inspects payload integrity and auto-heals (reload static bake, reset orphaned selection) without removing features. Closest public installs only. Open `/tracker`.
