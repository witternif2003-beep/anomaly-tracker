#!/usr/bin/env bash
# Post-doc pipeline smoke: orbital globe scene + postdoc-995500 virtual + TOP 500 SOTA + Live P1 + orbital globe pipeline diagnosis bake.
set -euo pipefail
...
for e in events[:5]:
    if not isinstance(e.get("lat"), (int, float)) or not isinstance(e.get("lon"), (int, float)):
        ...
        break
if postdoc.get("total") != 995500:
    errors.append(f"postdocCatalog expected total=995500, got {postdoc.get('total')}")
if not postdoc.get("virtualExpand"):
    errors.append("postdocCatalog.virtualExpand required for 995.5k Pages-safe catalog")
...
if summary.get("postdocImprovements") != 995500:
    errors.append("summary.postdocImprovements != 995500")
...
print(
    ...
    f"nodes={len(nodes)} events={len(events)} postdoc=995500 top500-sota live-p1 oneshot-install-inventory virtual",
