#!/usr/bin/env bash
# Post-doc pipeline smoke: orbital globe scene + postdoc-905500 virtual + TOP 500 SOTA + Live P1 + orbital globe pipeline diagnosis bake.
set -euo pipefail
...
for e in events[:5]:
    if not isinstance(e.get("lat"), (int, float)) or not isinstance(e.get("lon"), (int, float)):
        ...
        break
if postdoc.get("total") != 905500:
    errors.append(f"postdocCatalog expected total=905500, got {postdoc.get('total')}")
if not postdoc.get("virtualExpand"):
    errors.append("postdocCatalog.virtualExpand required for 905.5k Pages-safe catalog")
...
if summary.get("postdocImprovements") != 905500:
    errors.append("summary.postdocImprovements != 905500")
...
if (scout.get("repairRescanPasses") or 0) < 9:
    errors.append("scoutBot.repairRescanPasses < 9")
if scout.get("mode") not in ("postdoc-extreme-24x7", "postdoc-x3-extreme-24x7", "postdoc-x9-extreme-24x7"):
    errors.append("scoutBot.mode not postdoc-x9-extreme")
if scout.get("mode") != "postdoc-x9-extreme-24x7":
    errors.append("scoutBot.mode must be postdoc-x9-extreme-24x7")
if len(scout.get("healActions") or []) < 8:
    errors.append("scoutBot.healActions incomplete (<8)")
if (scout.get("tickMs") or 9999) > 22:
    errors.append("scoutBot.tickMs not ≤22 (×9 harder)")
if (scout.get("gateTarget") or 0) < 1215:
    errors.append("scoutBot.gateTarget < 1215")
...
if (code.get("gateCount") or 0) < 72:
    errors.append("scoutCodeIntegrity.gateCount < 72")
...
print(
    ...
    f"nodes={len(nodes)} events={len(events)} postdoc=905500 top500-sota live-p1 globe-pipeline-diagnosis virtual",
