#!/usr/bin/env bash
# Post-doc pipeline smoke: orbital globe scene + postdoc-635500 virtual + TOP 500 SOTA + Live P1 + BO auto-queue + chamber CRISP bake.
set -euo pipefail
...
for e in events[:5]:
    if not isinstance(e.get("lat"), (int, float)) or not isinstance(e.get("lon"), (int, float)):
        ...
        break
if postdoc.get("total") != 635500:
    errors.append(f"postdocCatalog expected total=635500, got {postdoc.get('total')}")
if not postdoc.get("virtualExpand"):
    errors.append("postdocCatalog.virtualExpand required for 635.5k Pages-safe catalog")
...
if summary.get("postdocImprovements") != 635500:
    errors.append("summary.postdocImprovements != 635500")
...
print(
    ...
    f"nodes={len(nodes)} events={len(events)} postdoc=635500 top500-sota live-p1 bo-auto-queue chamber-crisp virtual",
