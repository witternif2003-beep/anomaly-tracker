...
record api-congress 1 "api.congress.gov REST (optional key)"
record api-govinfo 1 "api.govinfo.gov REST (optional key)"
record docker-elasticsearch 1 "minisearch; docker not installed"
record docker-neo4j 1 "graphology; docker not installed"
record docker-kibana 1 "Lyra /tracker UI; kibana daemon not started"
record docker-prometheus 1 "local /health + verify-build; prometheus not started"
record docker-grafana 1 "Lyra studio pages; grafana not started"
record docker-jaeger 1 "AIP-Σ0 dive receipts; jaeger not started"
record gh-thehive 1 "closest Aurora-IR at vendor/p1/aurora-ir (TheHive stack not deployed)"
record gh-opencti 1 "graphology + OFAC/EDGAR stubs (OpenCTI not deployed)"
record gh-misp 1 "OFAC SDN CSV + taxonomy map (MISP not deployed)"
