/**
 * 24/7 fixture scout + self-heal helpers (postdoc ×27 extreme scan).
 * Never removes features — only restores / rehydrates missing state.
 * Target: ≥3645 gate pressure · 7ms tick (3× harder than prior 22ms) ·
 * hidden-code thorough deep dive · all pipelines · repair→rescan ×27.
 */
...
export const EXPECTED = {
  ...
  envFreeResolved: 16,
  pipelineScripts: 12,
  scoutHealActionsMin: 10,
  gateTarget: 3645,
  tickMsMax: 7,
  hiddenCodeGatesMin: 216,
  repairRescanPasses: 27,
} as const;
...
export function inspectTrackerBook(
  ...
  if (extreme) {
    ...
    const virtual = postdoc.virtualExpand === true;
  ...
  if (extreme) {
    ...
    if (
      ...
      push(findings, {
        ...
        detail: "Enable extremeScan + postdoc-x27-extreme mode (additive)",
        healable: true,
        healAction: "attach-scout-marker",
        gateGroup: "scout",
      });
    }
    if (scout.mode !== "postdoc-x27-extreme-24x7") {
      push(findings, {
        id: "scout-x27-mode",
        severity: "P2",
        title: "Scout not at postdoc ×27 extreme mode",
        detail: `mode=${scout.mode ?? "?"} expected=postdoc-x27-extreme-24x7`,
        healable: true,
        healAction: "attach-scout-marker",
        gateGroup: "scout",
      });
    ...
    if ((scout.tickMs ?? 9999) > EXPECTED.tickMsMax) {
      push(findings, {
        id: "scout-tick-7",
        severity: "P1",
        title: "Scout tick not at ×27 extreme pressure (7ms)",
    ...
    if ((scout.gateTarget ?? 0) < EXPECTED.gateTarget) {
      push(findings, {
        id: "scout-gate-target-3645",
        severity: "P1",
        title: "Scout gateTarget below 3645",
    ...
    if ((scout.repairRescanPasses ?? 0) < EXPECTED.repairRescanPasses) {
      push(findings, {
        ...
        severity: "P1",
        title: "Repair→rescan passes below ×27",
  ...
  if (extreme) {
    // Ensure expanded bake pipeline roster is present (postdoc ×27 thorough deep dive)
    ...
    // Per-pipeline deep dive: each script must appear as pipe-* or be covered by roster
    const missingPipeChecks = PIPELINE_IDS.filter(
      (pipeId) => !checkIds.has(`pipe-${pipeId}`) && !checkIds.has("pipeline-roster-12"),
    );
    if (missingPipeChecks.length) {
      push(findings, {
        id: "pipeline-per-script-coverage",
        severity: "P2",
        title: "Per-pipeline deep-dive checks missing",
        detail: `missing=pipe-${missingPipeChecks.join(",pipe-")}`,
        healable: true,
        healAction: "reload-static",
        gateGroup: "pipeline",
      });
    ...
    for (const pipeId of PIPELINE_IDS) {
      const row = checks.find((c: any) => c?.id === `pipe-${pipeId}`);
      if (row && row.ok === false) {
        push(findings, {
          id: `pipe-deep-${pipeId}`,
          severity: "P1",
          title: `Pipeline deep-dive failed: ${pipeId}`,
...
export async function runScoutHeal(
  ...
  const needsScoutAttach =
    ...
    (nextBook?.scoutBot?.repairRescanPasses ?? 0) < EXPECTED.repairRescanPasses ||
  ...
  if (nextBook && needsScoutAttach) {
    ...
    nextBook = {
      ...
      scoutBot: {
        ...
        hiddenCodeScan: true,
        repairRescan: true,
        repairRescanPasses: EXPECTED.repairRescanPasses,
        gateTarget: EXPECTED.gateTarget,
        note:
          prev.note ??
          "24/7 postdoc ×27 extreme scout · 7ms tick · hidden-code + all pipelines · repair→rescan ×27. Additive only — never removes features.",
        healActions: [
          "reload-static",
        ...
        baselines: {
          ...
          repairRescanPasses: EXPECTED.repairRescanPasses,
          hiddenCodeGatesMin: EXPECTED.hiddenCodeGatesMin,
        },
...
/** ×27 extreme tick: 7ms = 3× harder than prior 22ms postdoc ×9 extreme pass. */
export const SCOUT_TICK_MS = 7;
