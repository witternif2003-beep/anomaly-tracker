...
export const POSTDOC_TOTAL = 995_500;
...
export function buildPostdocImprovement(
  ...
  const forensicQuery = `FQ-${pad} · TRACKER · Live-P1 · Install-Inventory · ${axis.label} · ${entityType.label} · ${category.label} · artifact=${element.artifact}`;
  ...
  return {
    ...
    question: `${axis.prompt} Apply on /tracker (Live P1 telemetry + one-shot/install inventory + package-lock/MCP/credentials/command-output via tracker-3d-smoke + pipelineHealth) to ${entityType.label} under ${category.label} with artifact lens "${element.artifact}".`,
    method,
    falsifier,
    deliverable,
    ...
    wontDo,
    priority,
    pipelineCheck: `tracker-tab · live-p1-telemetry · oneshot-install-inventory · package-lock-mcp-credentials · postdoc-${total} · top500-sota · telemetry-24x7 · falsifier=${falsifier}`,
