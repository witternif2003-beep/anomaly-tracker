...
export const POSTDOC_TOTAL = 95_500;
...
export function buildPostdocImprovement(
  ...
  const forensicQuery = `FQ-${pad} · TRACKER · ${axis.label} · ${entityType.label} · ${category.label} · artifact=${element.artifact}`;
  ...
  return {
    ...
    question: `${axis.prompt} Apply on /tracker to ${entityType.label} under ${category.label} with artifact lens "${element.artifact}".`,
    method,
    falsifier,
    deliverable,
    ...
    wontDo,
    priority,
    pipelineCheck: `tracker-tab · postdoc-${total} · top500-sota · telemetry-24x7 · falsifier=${falsifier}`,
